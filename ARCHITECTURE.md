...
### HOLD + 比率連動サイレンス（疎なテレメトリ対策）

テレメトリは比率（Ratio）が高いほど疎になる（間隔 = 分母 × 2ms：1:16=32ms …
1:128=256ms）。報告窓ごとに複数サンプルを要求すると高比率で取りこぼすため:

- テレメトリを1つでも受信したら値を保持（HOLD）し、最後の受信時刻を記録（min=1）。
- 比率に応じたサイレンスタイムアウト（= 分母 × 2ms × 3、120〜600ms にクランプ。
  1:64→約384ms、1:128→600ms）を過ぎるまで保持値を報告し、過ぎたら floor に落とす。
- 比率は SYNC パケットから読むが、**ホップグリッドの再アンカーは行わない**
  （SYNC からの再アンカーは FHSS ロックを壊すため）。SYNC は UID 照合と比率取得のみ。

実用範囲: **1:64 まで安定、1:128（500Hz "Std"）も機能。**
