...
| 現 `main`（複数台対応版） | **TX比率 1:2 推奨**（密な捕捉で滑らか） | テレメトリ捕捉修正＋包絡線フィルタ、SX1280 ハング自動復旧、UID ゲート、4台同時運用、EMA α=0.25 |

> `main` は常に最新。各リリースはタグ代わりのブランチ（`1.0.0` / `1.0.1`）として保存しています。
>
> ※ 現 `main` は `1.0.1` 以降にテレメトリ**捕捉**を改善しました。実運用では **TX のテレメトリ比率を 1:2** に設定してください（密に取れて滑らか。理由は後述）。
