...
- テレメトリ Ratio -> HOLD + 比率連動サイレンス（min=1）で **1:64 まで安定、
  1:128（500Hz "Std"）も機能**。比率は SYNC から読むが**ホップグリッドは再アンカー
  しない**（再アンカーはロックを壊す）。
- EMA 平滑化係数 -> α = 0.25（gate_node/config.h）。
- デフォルト閾値 -> EnterAt -55dBm / ExitAt -62dBm（TX バックグラウンドより十分高く）。

未解決 / 今後の課題:
- 高比率（1:8 以上）の**飛行中**捕捉率向上。自由走行 8ms グリッドが TX 水晶と
  一致せず滞在位相がドリフトするため、疎なテレメトリの捕捉は本質的に難しい。
  watchdog 再ロックや SYNC 位相再アンカーは副作用（スタッター/悪化）が大きく不採用。
